...
public class ArmorStandPresetsPage extends AbstractArmorStandPage {
  ...
  public ArmorStandPresetsPage(MinecraftClient client, ArmorStandScreen screen) {
    super(client, screen, Text.translatable("armorstands.page.presets"), 4);
  }
  ...
  public void init() {
    ...
    screen.addDrawableChild(new MiniButtonWidget(
        ...
        Text.literal("A"),
        (button) -> {
          ClientNetworking.sendSetPosePacket(screen.getArmorStand(), PosePreset.ATTENTION);
        }));
    screen.addDrawableChild(new MiniButtonWidget(
        ...
        Text.literal("B"),
        (button) -> {
          ClientNetworking.sendSetPosePacket(screen.getArmorStand(), PosePreset.WALKING);
        }));
    screen.addDrawableChild(new MiniButtonWidget(
        refX,
        refY,
        ...
        Text.literal("C"),
        (button) -> {
          ClientNetworking.sendSetPosePacket(screen.getArmorStand(), PosePreset.RUNNING);
        }));
  }
}
