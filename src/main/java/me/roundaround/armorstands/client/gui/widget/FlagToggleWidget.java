...
public class FlagToggleWidget extends PressableWidget {
  ...
  public FlagToggleWidget(
    ...
    this.flagLabel = LabelWidget.builder(flag.getDisplayName(), x - WIDGET_WIDTH - 2 - valueLabelWidth - 2,
            y + height / 2
        )
        .justifiedRight()
        .alignedMiddle()
    ...
    this.valueLabel = LabelWidget.builder(
        ...
        x - valueLabelWidth / 2, y + height / 2
    ).justifiedCenter().alignedMiddle().shiftForPadding().build();
  ...
  private void renderControl(DrawContext drawContext, int mouseX, int mouseY, float delta) {
    ...
    drawContext.drawGuiTexture(getHandleTexture(), widgetX + offset, widgetY, BAR_WIDTH, WIDGET_HEIGHT);
  }
